...
export interface AdminOverview {
  ...
  recentEvents: Array<{ kind: string; sessionId: string | null; createdAt: Date; payload: unknown }>;
 ...
 * This therefore returns transcripts ONLY for sessions still in progress, and says so in the
 * payload rather than returning an empty field that looks like a bug.
 *
 ...
 *
 * Shared between the admin page (server-rendered) and the admin API route, so both read the same
 * data through the same query.
 */
