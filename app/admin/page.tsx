...
/**
 * The admin view. Everything the CHP's screen deliberately hides.
 *
 * This screen is aimed at whoever is evaluating whether the system works — a judge, a supervisor,
 * a clinician auditing the pipeline. It is in English, unlike every other screen, because it is
 * not used in a household.
 *
 * The section that matters most is the canaries: items the model produced and the code threw away.
 * A product that quotes a mother saying things she did not say fails silently unless someone is
 * counting, so the count is the first thing on the page after configuration.
 */

import Link from "next/link";
import { useCallback, useEffect, useState } from "react";
import { ADMIN_TOKEN_STORAGE_KEY } from "@/lib/roles";

interface AdminData {
  config: {
    asrProvider: string;
    saharaCorrections: string;
    llmProvider: string;
    llmModel: string;
    deletionFloor: { floor: number; derived: boolean; provenance: string; n?: number };
    safetyLexicon: { total: number; reviewed: number; allReviewed: boolean };
  };
  canaries: Record<string, number>;
  sessions: Array<{
    id: string;
    mother: string;
    chpCode: string;
    ...
    turnCount: number;
    startedAt: string;
    languageProfile: { sw: number; en: number; sheng: number } | null;
  }>;
  turns: Array<{
    sessionId: string;
    idx: number;
  ...
  records: Array<{
    ...
    chpCode: string;
    createdAt: string;
    scores: { phq9?: number; gad7?: number; phq9Band?: string };
    referral: { tier?: string; ruleApplied?: number; incomplete?: boolean };
    risk: { flagged?: boolean };
    disclaimers: string[];
  }>;
  auditByKind: Record<string, number>;
...
export default function Admin() {
  ...
  const load = useCallback(async (t: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(body.messageEn ?? "Unauthorized.");
        setData(null);
        return;
      }
  ...
  if (!data) {
    return (
      <main className="min-h-screen space-y-4 p-4">
        <h1 className="text-xl font-semibold">MAMA-SAUTI · admin</h1>
        <p className="text-sm text-neutral-700">
          The technical surface: pipeline telemetry, audit trail, model configuration. None of this
          appears on the CHP&apos;s screen.
        </p>
        <form
        ...
        >
          ...
          <p className="text-xs text-neutral-500">
            This is not authentication. It is a shared passphrase that keeps the technical surface
            off a CHP&apos;s phone and gates a demo view. Real auth is pilot work.
          </p>
        </form>
  ...
  return (
    <main className="min-h-screen space-y-6 p-4 pb-12">
      ...
      <section className="space-y-2">
        <h2 className="text-base font-semibold">Pipeline canaries</h2>
        <p className="text-sm text-neutral-700">
      ...
      {/* ---- Configuration ---- */}
      <section className="space-y-2">
        <h2 className="text-base font-semibold">Live configuration</h2>
      ...
      {/* ---- Per-turn telemetry ---- */}
      <section className="space-y-2">
        <h2 className="text-base font-semibold">Per-turn telemetry</h2>
      ...
      {/* ---- Sessions ---- */}
      <section className="space-y-2">
        <h2 className="text-base font-semibold">Sessions ({data.sessions.length})</h2>
      ...
      {/* ---- Records ---- */}
      <section className="space-y-2">
        <h2 className="text-base font-semibold">Screening records ({data.records.length})</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="text-left text-neutral-500">
              <tr>
      ...
      {data.anonymousCounters.length > 0 && (
        <section className="space-y-2">
          <h2 className="text-base font-semibold">Anonymous counters</h2>
          <p className="text-sm text-neutral-700">
